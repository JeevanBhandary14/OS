...
void main() {
    ...
    int n, time = 0, count = 0, current_process = -1;
    ...
    while (count < n) {
        ...
        for (int i = 0; i < n; i++) {
            if (at[i] <= time && remaining_burst[i] > 0) {
                if (smallest == -1 || at[i] < at[smallest]) {
                    smallest = i;
                }
            }
        }
        ...
        if (smallest == -1) {
            ...
            if (current_process != -1 && remaining_burst[current_process] > 0 && at[smallest] < at[current_process]) {
